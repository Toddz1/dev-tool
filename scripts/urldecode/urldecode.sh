...
if [ -t 0 ]; then
    # 终端输入模式
    if [ $# -ne 1 ]; then
        echo "Usage: cmd <encoded-string>" >&2
        echo "   or: echo <encoded-string> | cmd" >&2
        echo "Example: cmd hello%20world" >&2
        echo "     or: echo hello%20world | cmd" >&2
        exit 1
    fi
    DEV_TOOLS_INPUT="$1"
else
...
# 核心命令
echo -n "$DEV_TOOLS_INPUT" | $DEV_TOOLS_PYTHON -c "import sys, urllib.parse; print(urllib.parse.unquote(sys.stdin.read()));"
